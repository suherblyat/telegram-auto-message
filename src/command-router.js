...
export default {
  async fetch(request, env, ctx) {
    if (request.method !== "POST") return privateWorker.fetch(request, env, ctx);
    ...
    if (!message?.text) return privateWorker.fetch(request, env, ctx);
    ...
    if (isCommand(lower, ["/svpismo", "/свписмо", "/sveto_pismo", "/свето_писмо"])) {
      ...
      if (!args) return sendBibleHelp(message.chat.id, message.message_thread_id);
...
async function handleBibleLookup({ message, args }) {
  ...
  if (!parsed.ok) return sendBibleHelp(message.chat.id, message.message_thread_id, true);

  const result = await fetchBiblePassage(parsed.queryReference);
  if (!result.ok) {
    return sendGroupMessage(
      message.chat.id,
      `📖 <b>Свето Писмо</b>\n\nНисам успео да нађем цитат.\n\nРеференца: <code>${escapeHtml(parsed.displayReference)}</code>\nРазлог: ${escapeHtml(result.error || "непозната грешка")}`,
...
function parseBibleReference(input) {
  ...
  const verses = match[3].replace(/\s+/g, "").replace(/[.]/g, ",").trim();
...
function findBook(rawBook) {
  const key = normalizeBookInput(rawBook);
...
function normalizeBookInput(value) {
  let v = transliterateSerbian(String(value || "").toLowerCase())
    .replace(/[.]/g, " ")
    .replace(/\b(poslanica|poslanice|svetog|sv|apostola|apostol)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  ...
  v = v
    .replace(/\bpetrova\b|\bpetru\b|\bpetra\b|\bpeter\b|\bpetr\b|\bptr\b/g, "petar")
    .replace(/\bjovanova\b|\bjovanovu\b|\bjov\b/g, "jovan")
...
const BOOKS = {
  ...
  "matej": b("Matthew", "Матеј"), "mt": b("Matthew", "Матеј"),
  "marko": b("Mark", "Марко"), "mk": b("Mark", "Марко"),
  "luka": b("Luke", "Лука"), "lk": b("Luke", "Лука"),
  "jovan": b("John", "Јован"), "jn": b("John", "Јован"),
  "dela": b("Acts", "Дела апостолска"), "dap": b("Acts", "Дела апостолска"),
  "rimljanima": b("Romans", "Римљанима"), "rim": b("Romans", "Римљанима"),
